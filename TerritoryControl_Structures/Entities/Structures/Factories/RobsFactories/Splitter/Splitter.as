...
void onCollision(CBlob@ this, CBlob@ blob, bool solid)
{
	...
	{
		f32 count = blob.getQuantity();
		u16 result = Maths::Floor(count*0.5);
		if (isServer() && result > 0)
		{
			blob.server_SetQuantity(count-result);
			CBlob@ drop = server_CreateBlob(blob.getName(), blob.getTeamNum(), this.getPosition()+Vec2f(0,1));
			...
			blob.setPosition(this.getPosition() + Vec2f(this.isFacingLeft() ? -12.0f : 12.0f, -8.0f));
		}
		blob.setVelocity(Vec2f(this.isFacingLeft() ? -0.6f : 0.6f, -1.0f));
	}
	else
